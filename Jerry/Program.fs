...
let endpoint = IPEndPoint(IPAddress.Loopback, 10100)

do
use object = new Mutex(false, "24-TermApp-s1")
        ...
        | x when x = rSize ->
            ...
            if data.rType = RType.Request then
                let response = match data.v with
                               | MRequest.Present -> string <| mPresent ()
                               | MRequest.BCount -> string <| mButtonCount ()
                               | MRequest.HWheel -> string <| mHWheel ()
                               | MRequest.VWheel -> string <| mVWheel ()
                               | _ -> "Internal error"
